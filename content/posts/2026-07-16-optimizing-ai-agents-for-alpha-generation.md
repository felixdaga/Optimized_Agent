...
**1. Don't just run a single backtest.** Even with temperature 0, a frozen setup on identical data can produce different results run to run, due to inherent non-determinism in the LLM. Agentic capabilities amplify this stochasticity and can significantly affect investment outcomes; tool-heavy OpenClaw spread **~4.4×** wider than a plain chatbot-with-data baseline. **One backtest is one draw from that distribution**, not the setup's true skill. Nonetheless, agent configurations retain recognizable identities (82–100% twin match vs ~8% by chance) — noise around a persistent character, not pure chaos.

**2. More is not better — and smarter is not better either.** In our study, **chatbot-like agents (floor LLM) consistently beat more sophisticated variants like OpenClaw on performance metrics**; **Grok 4.5 scored below Grok 4.3** despite higher benchmark intelligence, with a higher hallucination rate. **Net gain from an agentic component or model swap ≈ capability gain − (base error × amplification)**. Popular extras — feedback loops, memory — often flip the sign negative.

**3. You can build a hypothetical optimal agent from first principles.** We **specified one configuration — the "ex-ante agent" —** which outperformed all previous configurations and became the only agent that cleared a credible significance bar (**NW *t* > 3**) for idiosyncratic IC. Common trends: memory/reflection often net-negative unless you can control noise; a grounded mid-tier model beat a "smarter" one that hallucinates more; split agent roles by information stream; **verify claims against the raw data** (not bull/bear debate); prefetch what you already know instead of open-ended tool loops. Just **don't give OpenClaw the keys to your portfolio yet!**

## Headline results from framework demonstration

**Same rating mandate, same scoring rule, same comparison set** — 9 baseline agents plus the ex-ante agent. We judge each by **how well its ratings predict next-quarter returns** after removing overlap with common style and sector bets. Each agent was **run 3 times** (full pass over all stocks and dates) and scores were averaged. Charts sorted by statistical strength (residual NW *t*).

{{< chart "main-residual-mean-ic.svg" "Residual IC (h1)" >}}

{{< chart "main-residual-nw-t.svg" "Residual NW t (h1)" >}}

**Read.** The **chatbot baseline** (MiMo) had the **strongest raw correlation** (0.167) — but only the **ex-ante design** passed a strict significance test (**NW *t* > 3**, at 3.24): edge that looks **stable across quarters**, not just riding quality/value factors or one lucky period. Every other setup sits at or below a conventional |*t*| ≈ 2.7; OpenClaw does not reach |*t*| ≈ 2.

**Before vs after factor controls.** For almost every setup, both correlation and significance **drop** once we remove exposure to common style and sector bets — raw strength partly just re-expresses factors the data already contain. Exception: chatbot baseline on Grok 4.3 ticks up slightly on *t* (1.53 → 1.70) while correlation still falls. **Ex-ante** is the standout: both metrics **rise** after controls (IC 0.113 → 0.139; NW *t* 2.31 → 3.24) — cleaner and more significant once factor overlap is removed.

**Trends:**

- **Chatbot baseline beat off-the-shelf “upgraded” agents** — MiMo chatbot 0.167 / NW *t* 2.67 vs OpenClaw 0.077 / 1.35 and trading agent 0.127 / 2.10.
- **Trading agent beat OpenClaw** — structured roles helped vs open tools, but both lost to the simple chatbot.
- **Higher benchmark IQ ≠ better picks** when hallucination rate is worse (Grok 4.5 vs 4.3 on the chatbot baseline).
